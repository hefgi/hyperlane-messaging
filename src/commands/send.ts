...
export async function sendMessage(answers: SendAnswer) {
    try {
        const provider = new ethers.JsonRpcProvider(answers.rpc);
        const wallet = new ethers.Wallet(answers.privateKey, provider);
        const contract = new ethers.Contract(getMailboxAddress(answers.origin), getMailboxABI(), wallet);
        const messageBytes = '0x' + Buffer.from(answers.message).toString('hex'); 
        console.log(`Message being dispatched... Please wait.`);
        const txResponse = await contract.dispatch(getDomain(answers.destination), answers.destinationAddress, messageBytes);
        const txReceipt = await txResponse.wait();
        console.log(`Message dispatched with tx hash: ${txReceipt.hash}`);
        console.log(`Check your meesage on our explorer: https://explorer.hyperlane.xyz/?search=${txReceipt.hash}`);
    } catch (err) {
